...
public class VoteQuestionServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (type == 1)
            voteUpQuestion(1, qid);
        else
            voteDownQuestion(1, qid);
    ...
    private void voteUpQuestion(int userId, int id) {
        // Note that the injected javax.xml.ws.Service reference as well as port objects are not thread safe.
        // If the calling of port operations may lead to race condition some synchronization is required.
        stackexchangews.StackExchangeWS port = service.getStackExchangeWSPort();
        port.voteUpQuestion(userId, id);
    }

    private void voteDownQuestion(int userId, int id) {
        // Note that the injected javax.xml.ws.Service reference as well as port objects are not thread safe.
        // If the calling of port operations may lead to race condition some synchronization is required.
        stackexchangews.StackExchangeWS port = service.getStackExchangeWSPort();
        port.voteDownQuestion(userId, id);
    }
